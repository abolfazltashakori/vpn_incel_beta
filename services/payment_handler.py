...
from services.marzban_service import *
...
payment_store = defaultdict(dict)
payment_store_lock = asyncio.Lock()
...
def register_payment_handlers(bot):
    ...
    bot.add_handler(CallbackQueryHandler(
        lifetime_buy_service,
        filters=filters.regex("^lifetime$")
    ), group=2)

    bot.add_handler(CallbackQueryHandler(
        unlimited_buy_service,
        filters=filters.regex("^unlimited$")
    ), group=2)

    bot.add_handler(CallbackQueryHandler(
        longtime_buy_service,
...
async def gift_code_menu(client, callback_query: CallbackQuery):
    keyboard = [
...
async def balance_increase_menu(client, callback_query: CallbackQuery):
    keyboard = [
...
async def buy_new_service_menu(client, callback_query: CallbackQuery):
    try:
        keyboard = [
...
async def normal_buy_service(client, callback_query: CallbackQuery):
    try:
        keyboard = [
...
async def lifetime_buy_service(client, callback_query: CallbackQuery):
    try:
        keyboard = [
...
async def unlimited_buy_service(client, callback_query: CallbackQuery):
    try:
        keyboard = [
...
async def longtime_buy_service(client, callback_query: CallbackQuery):
    try:
        keyboard = [
...
async def handle_package_selection(client, callback_query: CallbackQuery):
    try:
        package_id = callback_query.data
        if package_id not in package_details:
            await callback_query.answer("⚠️ بسته مورد نظر یافت نشد!", show_alert=True)
            return

        package = package_details[package_id]
...
async def back_to_category(client, callback_query: CallbackQuery):
    try:
        category = callback_query.data.split("_")[-1]
        handler = globals()[f"{category}_buy_service"]
...
async def back_to_vpn_menu(client, callback_query: CallbackQuery):
    try:
        await buy_new_service_menu(client, callback_query)
    except Exception as e:
        logger.error(f"Error in back_to_vpn_menu: {e}")
        await callback_query.message.edit_text("⚠️ خطا در بازگشت به منو اصلی!")

async def confirm_purchase(client, callback_query: CallbackQuery):
    try:
        ...
        package = package_details[package_id]
        balance = user_db.get_balance(user_id)
        ...
        try:
            # کسر موجودی
            user_db.balance_decrease(user_id, package["price"])
...
`{service['subscription_url'] or service['links'][0]}`
            """
            user_db.increment_purchase_count(user_id)
            user_db.increment_invoice_count(user_id)
            ...
            user_db.balance_increase(user_id, package["price"])
...
async def start_balance_increase(client, callback_query: CallbackQuery):
    user_id = callback_query.from_user.id
...
async def cancel_operation(client, callback_query: CallbackQuery):
    ...
    await money_managment(client, callback_query)

async def get_receipt(client, message: Message):
    user_id = message.from_user.id
...
async def approve_balance(client, callback_query: CallbackQuery):
...
async def reject_balance(client, callback_query: CallbackQuery):
    try:
...
async def process_gift_code(client, message: Message):
    user_id = message.from_user.id

    if user_id not in user_states or user_states[user_id].get("state") != "waiting_for_gift_code":
        return
...
async def apply_gift_code(client, callback_query: CallbackQuery):
    user_id = callback_query.from_user.id
    ...
    user_states[user_id] = {"state": "waiting_for_gift_code"}
